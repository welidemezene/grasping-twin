...
W3=$REPO/week3
CKPT=week3/checkpoints/stage19_final

run_in_container() {
    docker run --rm --gpus all -v "$REPO":/workspace -w /workspace/week3 "$IMAGE" "$@"
}

echo "=== waiting for stage19 training to finish ==="
while docker ps --format '{{.Names}}' | grep -qx stage19; do
    sleep 60
done
echo "training container is gone at $(date -u +%H:%M:%SZ)"

if [ ! -f "$W3/week3/checkpoints/stage19_final.zip" ]; then
    echo "!! stage19_final.zip MISSING -- training did not reach the end."
    echo "last 20 lines of the training log:"
    tail -20 "$W3/stage19_train.log"
    echo "checkpoints that DO exist:"
    ls -1 "$W3/week3/checkpoints/" | grep '^stage19' | tail -5
    exit 1
fi

echo
echo "=== 512-trial airborne check ==="
run_in_container check_airborne.py "$CKPT" airborne_s19 5 2>&1 \
    | grep -vE 'Warning|\[Error\]|deprecated' | tail -45

echo
echo "=== recorded replay (for the video) ==="
run_in_container record_motion_sticky.py "$CKPT" motion_s19_final.json 5 2>&1 \
    | grep -vE 'Warning|\[Error\]|deprecated' | tail -20

echo
echo "=== DONE ==="
ls -l "$W3"/airborne_s19* "$W3"/motion_s19_final* 2>/dev/null
